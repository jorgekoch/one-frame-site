...
const ServiceCard = styled.article<{ $index:number }>`position:relative;min-height:325px;padding:20px 20px 24px;background:rgba(255,255,255,.84);color:var(--color-black);border:2px solid var(--color-black);border-radius:0;box-shadow:5px 6px 0 var(--color-red);overflow:hidden;transform:rotate(${p=>[-1.1,.45,.8,-.55][p.$index]}deg);transition:transform .22s ease,box-shadow .22s ease,background-color .22s ease,color .22s ease;&:hover{transform:translateY(-6px) rotate(0deg);background:var(--color-black);color:var(--color-white);box-shadow:9px 11px 0 var(--color-red)}.service-top{display:flex;justify-content:space-between;align-items:flex-start;color:var(--color-red);font-size:11px;font-weight:900}.service-top img{width:48px;height:48px;object-fit:contain;transition:filter .22s ease}&:hover .service-top img{filter:invert(1)}h3{font:clamp(30px,2.6vw,39px)/.84 var(--font-display);text-transform:uppercase;letter-spacing:-.055em;margin:70px 0 18px;max-width:230px}p{font-size:12px;line-height:1.3;max-width:255px;opacity:.9}.plus{position:absolute;right:18px;bottom:14px;color:var(--color-red);font-size:22px;font-weight:900}`;
