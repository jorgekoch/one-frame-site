...
export function ClientsSection({ clients }: { clients: Client[] }) {
  return (
    <Clients>
      ...
      <ClientIntro>
        Algumas marcas que já colocaram suas histórias em movimento.
      </ClientIntro>

      <ClientGrid>
        {clients.map((client, index) => (
          <ClientName key={client} className={`client-${index + 1}`}>
            {client}
          </ClientName>
        ))}
      </ClientGrid>
...
const Texture = styled.img`
  ...
  filter: grayscale(1) contrast(1.2) rotate(3deg);
  pointer-events: none;
`;
...
const Clients = styled.section`
  ...
  h2 {
    position: relative;
    z-index: 3;
    font: clamp(58px, 7.4vw, 105px)/0.76 var(--font-display);
...
const ClientName = styled.span`
  position: relative;
  z-index: 2;
  font: clamp(20px, 2.5vw, 36px)/0.9 var(--font-display);
