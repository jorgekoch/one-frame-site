  ...
  return (
    <Sessions id="sessions">
      <SessionVisual>
        ...
        <img className="texture-secondary" src={`${A}lines-04.webp`} alt="" aria-hidden="true" loading="lazy" decoding="async" />
...
const SessionVisual = styled.div`position:relative;min-height:650px;overflow:hidden;background:var(--color-black);.texture-main,.texture-secondary{position:absolute;inset:-10%;width:120%;height:120%;object-fit:cover;filter:grayscale(1) contrast(1.65) brightness(.92);}.texture-main{opacity:.82;transform:rotate(-3deg) scale(1.08);}.texture-secondary{opacity:.24;transform:rotate(9deg) scale(1.18);mix-blend-mode:screen;}.red-overlay{position:absolute;inset:0;background:var(--color-red);mix-blend-mode:multiply;opacity:.38;}.paper-grain{position:absolute;inset:0;z-index:2;pointer-events:none;background-image:radial-gradient(rgba(255,255,255,.13) .7px,transparent .8px),radial-gradient(rgba(0,0,0,.16) .7px,transparent .8px);background-position:0 0,7px 9px;background-size:11px 11px,13px 13px;mix-blend-mode:soft-light;opacity:.18;}`;
